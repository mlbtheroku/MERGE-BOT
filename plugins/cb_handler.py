...
@Client.on_callback_query()
async def callback_handler(c: Client, cb: CallbackQuery):
    if cb.data.startswith("setMetadata_"):
        uid = int(cb.data.split("_")[1])
        await cb.message.edit(
    ...
    elif cb.data.startswith("editMetadata_"):
        uid = int(cb.data.split("_")[1])
        user = UserSettings(uid, cb.from_user.first_name)
        metadata_display = f"""
<b><u>Current Metadata for <a href='tg://user?id={uid}'>{cb.from_user.first_name}</a></u></b>
    ┃
    ┣**Author: <u>{user.author or "Not Set"}</u>**
    ┣**Video: <u>{user.video or "Not Set"}</u>**
    ┣**Audio: <u>{user.audio or "Not Set"}</u>**
    ┗**Subtitle: <u>{user.subtitle or "Not Set"}</u>**
"""
        markup = b.makebuttons(
            ["Edit", "Delete", "Back", "Close"],
            [f"editMetadataDetails_{uid}", f"deleteMetadata_{uid}", "back", "close"],
            rows=2
        )
        await cb.message.edit(
    ...
    elif cb.data.startswith("editMetadataDetails_"):
        uid = int(cb.data.split("_")[1])
        await cb.message.edit(
            text="Send your metadata in the format: author|video|audio|subtitle\n\nTime Out = 1 Minute",
            reply_markup=InlineKeyboardMarkup(
                [
                    [InlineKeyboardButton("Back", callback_data=f"back_{uid}"), InlineKeyboardButton("Close", callback_data="close")]
                ]
        ...
        # Set listener for metadata input
        metadata_msg = await c.listen(
        ...
        if metadata_msg:
            metadata_parts = metadata_msg.text.split('|')
            if len(metadata_parts) == 4:
                user = UserSettings(uid, cb.from_user.first_name)
                user.author = metadata_parts[0]
                user.video = metadata_parts[1]
                user.audio = metadata_parts[2]
                user.subtitle = metadata_parts[3]
                user.edit_metadata = True
                user.set()
                await metadata_msg.reply_text("Metadata updated successfully!")
                await userSettings(cb.message, uid, cb.from_user.first_name, cb.from_user.last_name, user)
        return

    elif cb.data.startswith("deleteMetadata_"):
        uid = int(cb.data.split("_")[1])
        user = UserSettings(uid, cb.from_user.first_name)
        user.author = None
        user.video = None
        user.audio = None
        user.subtitle = None
        user.edit_metadata = False
        user.set()
        await cb.message.reply_text("Metadata deleted successfully!")
        await userSettings(cb.message, uid, cb.from_user.first_name, cb.from_user.last_name, user)
        return

    elif cb.data.startswith("back_"):
        uid = int(cb.data.split("_")[1])
        await userSettings(cb.message, uid, cb.from_user.first_name, cb.from_user.last_name, UserSettings(uid, cb.from_user.first_name))
        return
